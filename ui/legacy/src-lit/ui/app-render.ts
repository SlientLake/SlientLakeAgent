...
const lazyChatroom = createLazy(() => import("./views/chatroom.ts"));
const lazyTopology = createLazy(() => import("./views/topology.ts"));
const lazyMcpManager = createLazy(() => import("./views/mcp-manager.ts"));
const lazyKbManager = createLazy(() => import("./views/kb-manager.ts"));
              ...
              <div class="sidebar-brand">
                ${
                  navCollapsed
                    ? nothing
                    : html`
                        <img class="sidebar-brand__logo" src="${agentLogoUrl(basePath)}" alt="SilentLake" />
                        <span class="sidebar-brand__copy">
                          <span class="sidebar-brand__eyebrow">${t("nav.control")}</span>
                          <span class="sidebar-brand__title">SilentLake</span>
                        </span>
                      `
                }
              </div>
            ...
            <div class="sidebar-shell__footer">
              <div class="sidebar-utility-group">
                <a
                  ...
                  href="https://github.com/SlientLake/SlientLakeAgent/tree/main/docs"
                ...
                m.renderAgents({
                  ...
                  onRefresh: async () => {
                    ...
                    if (state.agentsPanel === "skills" && refreshedAgentId) {
                      ...
                      void loadAgentFileContent(state, refreshedAgentId, "skills.json");
                    }
                  ...
                  onSelectPanel: (panel) => {
                    ...
                    if (panel === "skills") {
                      if (resolvedAgentId) {
                        ...
                        void loadAgentFileContent(state, resolvedAgentId, "skills.json");
                      }
                    }
                  ...
                  onAgentSkillModelChange: (agentId, skillName, field, value) => {
                    if (!resolvedAgentId || resolvedAgentId !== agentId) {
                      return;
                    }
                    const current = state.agentFileContents["skills.json"] ?? "{}";
                    ...
                    if (value) {
                      ...
                      delete skillEntry[field];
                    }
                    if (Object.keys(skillEntry).length > 0) {
                      parsed[skillName] = skillEntry;
                    } else {
                      delete parsed[skillName];
                    }
                    const next = JSON.stringify(parsed, null, 2);
                    state.agentFileContents = { ...state.agentFileContents, "skills.json": next };
                    state.agentFileDrafts = { ...state.agentFileDrafts, "skills.json": next };
                    void saveAgentFile(state, agentId, "skills.json", next);
                  },
        ...
        ${
          state.tab === "appearance"
            ? renderConfig({
                ...
                setLocale: (l) => void i18n.setLocale(l),
                gatewayUrl: state.settings.gatewayUrl,
                assistantName: state.assistantName,
                configPath: state.configSnapshot?.path ?? null,
