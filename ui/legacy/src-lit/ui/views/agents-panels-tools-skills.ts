...
// Coding CLI skills that support model configuration
const CODING_CLI_SKILLS = new Set(["coding-agent", "gemini"]);

// Preferred CLI options for coding-agent
const CODING_AGENT_CLI_OPTIONS = ["claude", "codex", "opencode", "pi"] as const;
...
export function renderAgentSkills(params: {
  ...
  configDirty: boolean;
  filter: string;
  skillsJsonContent: string | null;
  skillsFileSaving: boolean;
  ...
  onConfigReload: () => void;
  onConfigSave: () => void;
  onSkillModelChange: (agentId: string, skillName: string, field: string, value: string) => void;
}) {
  ...
  const skillsModels = parseSkillsJson(params.skillsJsonContent);
              ...
              <div class="agent-skills-groups" style="margin-top: 16px;">
                ${groups.map((group) =>
                  renderAgentSkillGroup(group, {
                    ...
                    editable,
                    skillsModels,
                    skillsFileSaving: params.skillsFileSaving,
                    onToggle: params.onToggle,
                    onSkillModelChange: params.onSkillModelChange,
                  }),
                )}
              </div>
            `
...
function renderAgentSkillGroup(
  ...
  params: {
    ...
    editable: boolean;
    skillsModels: SkillModelsConfig;
    skillsFileSaving: boolean;
    onToggle: (agentId: string, skillName: string, enabled: boolean) => void;
    onSkillModelChange: (agentId: string, skillName: string, field: string, value: string) => void;
  },
      ...
      <div class="list skills-grid">
        ${group.skills.map((skill) =>
          renderAgentSkillRow(skill, {
            ...
            skillsModels: params.skillsModels,
            skillsFileSaving: params.skillsFileSaving,
            onToggle: params.onToggle,
            onSkillModelChange: params.onSkillModelChange,
          }),
...
function renderAgentSkillRow(
  ...
  const isCodingSkill = CODING_CLI_SKILLS.has(skill.name);
  const skillConfig = params.skillsModels[skill.name] ?? {};
    ...
    <div class="list-item agent-skill-row">
      <div class="list-main">
        ...
        ${
          isCodingSkill
            ? html`
                <details class="skill-model-config" style="margin-top: 8px;">
                  <summary class="skill-model-config__toggle">⚙ Model config</summary>
                  <div class="skill-model-config__body">
                    <label class="skill-model-config__field">
                      <span class="skill-model-config__label">Model override</span>
                      <input
                        class="skill-model-config__input"
                        type="text"
                        placeholder="e.g. claude-opus-4-5, gemini-2.5-pro"
                        .value=${skillConfig.model ?? ""}
                        ?disabled=${params.skillsFileSaving}
                        @change=${(e: Event) =>
                          params.onSkillModelChange(
                            params.agentId,
                            skill.name,
                            "model",
                    ...
                    ${
                      skill.name === "coding-agent"
                        ? html`
                            <label class="skill-model-config__field">
                              <span class="skill-model-config__label">Preferred CLI</span>
                              <select
                                class="skill-model-config__input"
                                .value=${skillConfig.preferredCli ?? ""}
                                ?disabled=${params.skillsFileSaving}
                                @change=${(e: Event) =>
                                  params.onSkillModelChange(
                                    params.agentId,
                              ...
                              >
                                ...
                                ${CODING_AGENT_CLI_OPTIONS.map(
                                  (cli) => html`<option value=${cli}>${cli}</option>`,
                                )}
                              </select>
                            </label>
                          `
