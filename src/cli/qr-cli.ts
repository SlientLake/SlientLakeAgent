...
export function registerQrCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "github.com/SlientLake/SlientLakeAgent/tree/main/docs/cli/qr")}\n`,
    )
    .option(
      "--remote",
    ...
    .action(async (opts: QrCliOptions) => {
      try {
        ...
        const lines: string[] = [
          ...
          "Scan this with the SilentLake iOS app (Onboarding -> Scan QR).",
          "",
        ];
