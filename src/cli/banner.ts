...
import { getTerminalLocale } from "../terminal/locale.js";
...
export function formatCliBannerLine(version: string, options: BannerOptions = {}): string {
  ...
  const rawTagline = pickTagline({ ...options, mode: resolveTaglineMode(options) });
  // Use zh-CN tagline hint when locale is zh-CN
  const tagline =
    getTerminalLocale() === "zh-CN" && !rawTagline ? "多 Agent 协作平台 · SilentLake" : rawTagline;
  const rich = options.richTty ?? isRich();
  const title = "🌊 SilentLake";
  const prefix = "🌊 ";
...
export function formatCliBannerArt(options: BannerOptions = {}): string {
  ...
  const colored = LOBSTER_ASCII.map((line) => {
    if (line.includes("SILENTLAKE")) {
      return (
        theme.muted("               ") +
        theme.accent("🌊") +
        theme.info(" SILENTLAKE ") +
        theme.accent("🌊")
      );
    }
