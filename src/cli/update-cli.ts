...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update SilentLake and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["silentlake update", "Update a source checkout (git)"],
        ["silentlake update --channel beta", "Switch to beta channel (git + npm)"],
        ["silentlake update --channel dev", "Switch to dev channel (git + npm)"],
        ["silentlake update --tag beta", "One-off update to a dist-tag or version"],
        ["silentlake update --tag main", "One-off package install from GitHub main"],
        ["silentlake update --dry-run", "Preview actions without changing anything"],
        ["silentlake update --no-restart", "Update without restarting the service"],
        ["silentlake update --json", "Output result as JSON"],
        ["silentlake update --yes", "Non-interactive (accept downgrade prompts)"],
        ["silentlake update wizard", "Interactive update wizard"],
        ["silentlake --update", "Shorthand for silentlake update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run silentlake update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "github.com/SlientLake/SlientLakeAgent/tree/main/docs/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["silentlake update status", "Show channel + version status."],
          ["silentlake update status --json", "JSON output."],
          ["silentlake update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "github.com/SlientLake/SlientLakeAgent/tree/main/docs/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
