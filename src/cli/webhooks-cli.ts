...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "github.com/SlientLake/SlientLakeAgent/tree/main/docs/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + SilentLake hooks")
    ...
    .option("--hook-url <url>", "SilentLake hook URL")
    .option("--hook-token <token>", "SilentLake hook token")
