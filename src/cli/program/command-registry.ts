...
const coreEntries: CoreCliEntry[] = [
  ...
  },
  {
    commands: [
      {
        name: "browser",
        description: "Manage SilentLake's dedicated browser (Chrome/Chromium)",
        hasSubcommands: true,
      },
    ],
    register: async ({ program }) => {
  ...
  // SilentLake 二开命令
  {
    commands: [
      {
        name: "topology",
        description: "显示 Agent 组织拓扑（Mermaid 或表格）",
        hasSubcommands: false,
      },
    ],
    register: async ({ program }) => {
      const mod = await import("../topology-cli.js");
      mod.registerTopologyCli(program);
    },
  },
  {
    commands: [
      {
        name: "report",
        description: "查看 Agent 汇报记录",
        hasSubcommands: true,
      },
    ],
    register: async ({ program }) => {
      const mod = await import("../report-cli.js");
      mod.registerReportCli(program);
    },
  },
  {
    commands: [
