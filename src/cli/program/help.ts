...
const EXAMPLES = [
  ["silentlake models --help", "Show detailed help for the models command."],
  [
    "silentlake channels login --verbose",
    "Link personal WhatsApp Web and show QR + connection logs.",
  ],
  [
    'silentlake message send --target +15555550123 --message "Hi" --json',
  ...
  [
    'silentlake agent --to +15555550123 --message "Run summary" --deliver',
    "Talk directly to the agent using the Gateway; optionally send the WhatsApp reply.",
  ],
  [
    'silentlake message send --channel telegram --target @mychat --message "Hi"',
...
export function configureProgramHelp(program: Command, ctx: ProgramContext) {
  ...
  if (
    ...
    console.log(
      commit ? `SilentLake ${ctx.programVersion} (${commit})` : `SilentLake ${ctx.programVersion}`,
    );
    process.exit(0);
  }
  ...
  program.addHelpText("afterAll", ({ command }) => {
    ...
    const docs = formatDocsLink("/cli", "github.com/SlientLake/SlientLakeAgent/tree/main/docs/cli");
